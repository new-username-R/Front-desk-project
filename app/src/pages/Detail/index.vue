    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :BigImage="BigImage" />
          <!-- 小图列表 -->
          <Scroll :skuImageList="skuImageList" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl
                v-for="(attrValue, index) in spuSaleAttrList"
                :key="attrValue.id"
              >
                <dt class="title">{{ attrValue.saleAttrName }}：</dt>
                <dd
                  v-for="(colorItem, index) in attrValue.spuSaleAttrValueList"
                  :key="colorItem.id"
                  :class="{ checkedColor: colorItem.isChecked == '1' }"
                  @click="changeChecked(colorItem,attrValue.spuSaleAttrValueList)"
                  ref="changedd"
                >
                  {{ colorItem.saleAttrValueName }}
                </dd>
...
import Scroll from "@/pages/Detail/Scroll";
import { mapState, mapGetters } from "vuex";
export default {
  name: "detail",
  components: { Zoom, Scroll },
  data() {
    return {
      skuNum: 1,
    };
  ...
  computed: {
    ...
    totalPrice() {
      return this.skuInfo.price * this.skuNum;
    },
  },
  methods: {
    inputVal() {
      ...
      if (isNaN(inputValue * 1)) {
        ...
        this.$refs.input.value = parseInt(this.$refs.input.value);
        this.skuNum = this.$refs.input.value;
      }
    },
    changeChecked(colorItem,arr) {
        // console.log(arr);
        arr.forEach(item => {
          item.isChecked="0"
        });
        colorItem.isChecked='1'
    },
    async addCart() {
      // sessionStorage.skuInfo=this.skuInfo
      // this.$router.push({name:"CartlList"})
      try {
