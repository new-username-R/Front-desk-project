      ...
      <div class="checkout-tit">
        ...
        <div class="paymark">
          ...
          <span class="fr"
            ...
            ><em class="orange money">￥{{ payMessage.totalFee }}</em></span
          >
        </div>
      </div>
...
export default {
  ...
  methods: {
    async getData() {
      const result =
        (await this.$API.reqPaymentInfo(this.$route.params.orderId)) || {};
      if (result.code==200) {
        this.payMessage = result.data;
      }
    },
    async open() {
      let url = await QRCode.toDataURL(this.payMessage.codeUrl);
      ...
      if (!this.timer) {
        ...
        this.timer = setInterval(async () => {
          let result = await this.$API.reqPayState(this.$route.params.orderId);
          console.log(result);
          if (result.code == 200) {
            return result.data;
            // this.$router.push("/paySuccess");
